...
const App: React.FC = () => {
  ...
  const handleAddHabit = (name: string) => {
    ...
    try {
      habitService.createHabit(authState.user.id, name);
      refreshHabits();
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const handleToggleHabit = (habitId: string) => {
    ...
    habitService.toggleHabitLog(habitId, today);
    refreshHabits();
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-900 text-neutral-100 selection:bg-emerald-500 selection:text-black">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <header className="mb-14">
          ...
          <h2 className="text-5xl font-black text-white tracking-tighter mb-4">{t('daily_three')}</h2>
        ...
        <div className="mb-16">
          <HabitForm onAdd={handleAddHabit} disabled={habits.length >= 3} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {habits.length === 0 && (
              <div className="col-span-full bg-neutral-800/50 border-2 border-dashed border-neutral-800 rounded-3xl py-24 flex flex-col items-center justify-center text-center">
        ...
        <footer className="mt-24 border-t border-neutral-800 pt-12 text-center pb-20">
